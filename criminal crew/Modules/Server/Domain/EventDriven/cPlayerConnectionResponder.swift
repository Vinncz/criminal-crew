...
extension PlayerConnectionResponder : GPHandlesEvents {
    ...
    public func placeSubscription ( on eventType: any GamePantry.GPEvent.Type ) {
        guard let relay = self.relay else {
            debug("PlayerConnectionResponder is unable to place subscription: relay is missing or not set"); return
        }
        
        guard let eventRouter = relay.eventRouter else {
            debug("PlayerConnectionResponder is unable to place subscription: eventRouter is missing or not set"); return
        }
...
extension PlayerConnectionResponder : GPEmitsEvents {
    ...
    public func emit ( _ event: GPEvent ) -> Bool {
        guard let relay = self.relay else {
            debug("PlayerConnectionResponder is unable to emit: relay is missing or not set"); return false
        }
        
        guard let eventRouter = relay.eventRouter else {
            debug("PlayerConnectionResponder is unable to emit: eventRouter is missing or not set"); return false
        }
...
extension PlayerConnectionResponder {
    ...
    private func handleAcquaintanceEvent ( _ event: GPAcquaintanceStatusUpdateEvent ) {
        guard let relay = self.relay else {
            debug("PlayerConnectionResponder is unable to handle events: relay is missing or not set"); return
        }
        
        guard let playerRuntimeContainer = relay.playerRuntimeContainer else {
            debug("PlayerConnectionResponder is unable to handle events: playerRuntimeContainer is missing or not set"); return
        }
        
        playerRuntimeContainer.update(event.subject, state: event.status)
        debug("PlayerConnectionResponder updates a player's state: \(event.subject) to \(event.status)")
    }
